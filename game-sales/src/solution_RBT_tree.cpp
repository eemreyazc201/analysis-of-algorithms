...
class RB_tree {
	...
	public:	
		...
		Node* RB_insert (Node* root, Node*& ptr) {
			Node* parent = NULL; Node* temp = root;

			while (temp != NULL) {
				parent = temp;

				if (compareAlphabetically(ptr->key, temp->key) == SMALLER) {temp = temp->left;}
				else if (compareAlphabetically(ptr->key, temp->key) == BIGGER) {temp = temp->right;}
				else if (compareAlphabetically(ptr->key, temp->key) == EQUAL) {
					temp->key.na_sales += ptr->key.na_sales; temp->key.eu_sales += ptr->key.eu_sales; temp->key.others_sales += ptr->key.others_sales;
					
					if (best_seller[0] == NULL || temp->key.na_sales > best_seller[0]->na_sales) {best_seller[0] = &temp->key;}
					if (best_seller[1] == NULL || temp->key.eu_sales > best_seller[1]->eu_sales) {best_seller[1] = &temp->key;}
					if (best_seller[2] == NULL || temp->key.others_sales > best_seller[2]->others_sales) {best_seller[2] = &temp->key;}
		...
		void insertValue (vector<string> n) {
			...
			Node* ptr = new Node(temp); root = RB_insert(root, ptr); RB_insert_fixup(ptr);
		}

        void RB_left_rotate (Node *ptr) {
			Node* child = ptr->right; ptr->right = child->left;
			if (child->left != NULL) {child->left->parent = ptr;}
        ...
        void RB_right_rotate (Node *ptr) {
			...
			if (child->right != NULL) {child->right->parent = ptr;}
			child->parent = ptr->parent;
			if (ptr->parent == NULL) {root = child;} else if (ptr == ptr->parent->right) {ptr->parent->right = child;} else {ptr->parent->left = child;}
			...
			while (ptr != root && ptr->get_color() == 1 && ptr->parent->get_color() == 1) {
				...
				if (parent == grandParent->left) {
					...
					if (uncle != NULL && uncle->get_color() == 1) {
						...
						RB_right_rotate(grandParent); int temp = parent->color; parent->color = grandParent->color; grandParent->color = temp; ptr = parent;
					}
				} else {
					...
					if (uncle != NULL && uncle->get_color() == 1) {
						...
						RB_left_rotate(grandParent); int temp = parent->color; parent->color = grandParent->color; grandParent->color = temp; ptr = parent;
					}
				}
			} root->set_color(0);   
        }
        ...
        void preorder () {
			...
			while (!stk.empty()) {
				...
				cout << string (stk.size(), '-') << node->key.name << endl;
