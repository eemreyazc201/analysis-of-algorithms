...
static MunitResult test_get_max (const MunitParameter params[], void* data) {
  ...
  auto maxAgeItem = std::max_element(items.begin(), items.end(), [](const Item& a, const Item& b) { return a.age < b.age; });
...
static MunitResult test_counting_sort_age_ascending (const MunitParameter params[], void* data) {
  ...
  for (int i = 0; i < itemSize; i++) {munit_assert_int(sorted[i].age, ==, sorted_items[i].age);}
...
static MunitResult test_counting_sort_age_descending (const MunitParameter params[], void* data) {
  ...
  int itemSize = items.size();
  // Verify the order of the age after sorting (ascending)
  for (int i = 0; i < itemSize; i++) {munit_assert_int(sorted[i].age, ==, sorted_items[itemSize - 1 - i].age);}
...
bool checkAlmostEqual (double a, double b, int i=0, double epsilon = 1e-5) {
    ...
    if (diff > epsilon) {std :: cout << "a: " << a << " b: " << b << " diff: " << diff <<  " i: " << i << std::endl;}
    return diff <= epsilon;
}
...
static MunitResult test_calculate_rarity_scores (const MunitParameter params[], void* data) {
  ...
  int itemSize = items.size();
  // Verify the order of the age after sorting (ascending)
  for (int i = 0; i < itemSize; i++) {munit_assert(checkAlmostEqual(sorted[i].rarityScore, items_rarity_calculated[i].rarityScore, i));}
...
static MunitResult test_heap_sort_rarity_descending (const MunitParameter params[], void* data) {
  ...
  for (int i = 0; i < itemSize; i++) {munit_assert(checkAlmostEqual(sorted[i].rarityScore, sorted_items[i].rarityScore, i));}
...
static MunitResult test_heap_sort_rarity_ascending (const MunitParameter params[], void* data) {
  ...
  for (int i = 0; i < itemSize; i++) {munit_assert(checkAlmostEqual(sorted[i].rarityScore, sorted_items[itemSize - 1 - i].rarityScore, itemSize - 1 - i));}
